...
Partial Class Reports_and_Query_t_notice_of_award
...
#End Region
    ...
    Protected Sub LoadSearching()
        Select Case (RadioButtonList1.SelectedIndex)
            Case 0
                dtNotice = objDerived.GetDataTable("SELECT * FROM [dbo].[View_RQ_AwardNotice] WHERE project_reference_no LIKE '%" & txtRefNumber.Text & "%'", CommandType.Text)
                grdNotice.DataSource = dtNotice
                grdNotice.DataBind()

            Case 1
                dtNotice = objDerived.GetDataTable("SELECT * FROM [dbo].[View_RQ_AwardNotice] WHERE Supplier_Id = '" & ddSupplier.SelectedItem.Value & "'", CommandType.Text)
                grdNotice.DataSource = dtNotice
                grdNotice.DataBind()

            Case 2
                dtNotice = objDerived.GetDataTable("SELECT * FROM [dbo].[View_RQ_AwardNotice] WHERE NOA_Date BETWEEN '" & txtdatefrom.Text & "' AND '" & txtdateto.Text & "'", CommandType.Text)
                grdNotice.DataSource = dtNotice
                grdNotice.DataBind()
    ...
    Protected Sub grdNotice_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdNotice.SelectedIndexChanged
        ...
        Session("Supplier_Id") = grdNotice.SelectedDataKey("Supplier_ID")
