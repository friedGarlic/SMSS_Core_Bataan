...
Partial Class Reports_and_Query_WasteMaterials_Reports
    ...
    Private Property dtWaste() As DataTable
        ...
        End Set
    End Property
    Public Function tempTable_WMR(ByVal row As Integer, ByVal sourceTable As DataTable) As DataTable
        Dim dt As DataTable = sourceTable.Clone()
        Dim dr As DataRow
    ...
    Private Sub Reports_and_Query_WasteMaterials_Reports_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            dtWaste = objDerived.GetDataTable("SELECT A.WMHdr_ID, A.WM_Date, A.ctrl_no, CASE WHEN A.Function_ID = 86 THEN B.RC_Name ELSE B.Function_Desc END AS RC_Name, CONVERT(BIT,1) AS isVisible    " &
                                                  ...
                                                  "  ORDER BY A.WM_Date DESC, A.ctrl_no DESC", CommandType.Text)

            If dtWaste.Rows.Count < 5 Then
                dtWaste.Merge(tempTable_WMR(4 - dtWaste.Rows.Count, dtWaste))
            End If
