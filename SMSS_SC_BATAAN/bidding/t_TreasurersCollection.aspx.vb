...
Partial Class bidding_t_TreasurersCollection
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            Dim MOP As Integer = objDerived.GetValue("Select mode_of_procurement_id from ams.mode_of_procurement where mode_description='Public Bidding'", CommandType.Text)
    ...
    Protected Sub grdListOfTransaction_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdListOfTransaction.SelectedIndexChanged
        ...
        If IsTransactionSelected() Then
            SetupForSelectedTransaction()
        Else
            SetupForNoTransaction()
        End If
    End Sub
    ...
    Private Function IsTransactionSelected() As Boolean
        Return Convert.ToInt32(grdListOfTransaction.SelectedDataKey("pre_procurement_hdr_id")) <> 0
    End Function

    Private Sub SetupForSelectedTransaction()
        lblProjectName.Text = Convert.ToString(grdListOfTransaction.SelectedDataKey("project_name"))
        lbllocation.Text = Convert.ToString(grdListOfTransaction.SelectedDataKey("BidLocation"))
        lblBiddocument.Text = FormatNumber(Convert.ToDecimal(grdListOfTransaction.SelectedDataKey("bid_docs")), 2)
    ...
    Private Sub SetupForNoTransaction()
        btnNew.Enabled = False
        ClearLabels()
        ClearAndDisableDropdown(drpSupplierList)

        ' Bind a default empty table with two rows if no transaction is selected
        BindDefaultSuppliersList()
    End Sub

    Private Sub ClearLabels()
        lblProjectName.Text = String.Empty
        lbllocation.Text = String.Empty
        lblBiddocument.Text = String.Empty
    End Sub

    Private Sub BindSupplierDropdown()
        Dim dtSuppliers As DataTable = objDerived.GetDataTable("SELECT * FROM dbo.Supplier ORDER BY SuppName", CommandType.Text)
        With drpSupplierList
    ...
    Private Sub ClearAndDisableDropdown(dropdown As DropDownList)
        dropdown.DataSource = Nothing
        dropdown.DataBind()
        dropdown.Items.Insert(0, "Select")
        dropdown.Enabled = False
    End Sub

    Private Sub BindDefaultSuppliersList()
        grdListOfSupplier.DataSource = createdatatable1(2)  ' Assumes this method returns a DataTable with two empty/default rows
        grdListOfSupplier.DataBind()
    End Sub
