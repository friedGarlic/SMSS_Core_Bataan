...
Partial Class bidding_Eligibility
...
#End Region
    ...
    Protected Sub LoadPage()
        dtProjects = objDerived.GetDataTable("SELECT DISTINCT A.pre_procurement_hdr_id, A.project_reference_no, E.Project_name, E.BidOpening_Place, A.ABC                                               " &
                                ...
                                " INNER JOIN AMS.obr_evaluation_dtl AS C ON B.obr_evaluation_dtl_id = C.obr_evaluation_dtl_id INNER JOIN AMS.ITB_Dtl AS D ON C.prhdr_id = D.prhdr_id                    " &
                                " INNER JOIN AMS.ITB_Hdr AS E ON D.ITB_Hdr_ID = E.ITB_Hdr_ID WHERE A.isPublicInfra = 0 AND A.withBid = 0 AND A.pre_procurement_hdr_id NOT IN (SELECT DISTINCT X.pre_procurement_hdr_id FROM AMS.bid_opening_hdr AS X WHERE X.isPass = 1) " &
    ...
    'Protected Sub LoadTabs()
    '    Try
    '        Dim procurementId As Integer = Convert.ToInt32(Session("pre_procurement_hdr_id"))

    '        If procurementId = 0 Then
    '            SetupTabsForNoProcurement()
    '        Else
    '            SetupTabsForExistingProcurement()
    '        End If

    '    Catch ex As Exception
    '        ' Hide specific columns or controls based on the context of the error
    '        If grdEqligibility IsNot Nothing AndAlso grdEqligibility.Columns.Count > 6 Then
    '            grdEqligibility.Columns(6).Visible = False
    '        End If
    '        ' Log error or send notification to the admin if necessary.
    '        ' LogError(ex) ' Implement this method based on your error logging strategy
    '        MsgeBox.CreateMessageAlertInUpdatePanel(UpdatePanel1, "Something went wrong during the process, please contact system admin.")
    '    End Try
    'End Sub

    Private Sub SetupTabsForNoProcurement()
        If btnTab1_Eligibility.CssClass = "TabButton_Active" Then
            InitializeEligibilityForNoProcurement()
        ElseIf btnTab2_Technical.CssClass = "TabButton_Active" Then
            InitializeTechnicalForNoProcurement()
        ElseIf btnTab3_Summary.CssClass = "TabButton_Active" Then
            InitializeSummaryForNoProcurement()
        End If
    End Sub

    Private Sub SetupTabsForExistingProcurement()
        If btnTab1_Eligibility.CssClass = "TabButton_Active" Then
            LoadEligibility()
            btnSaveEligibility.Enabled = True
    ...
    Private Sub InitializeEligibilityForNoProcurement()
        grdEqligibility.DataSource = CreateTable2(2)
        grdEqligibility.DataBind()
        btnSaveEligibility.Enabled = False
        mvTabs.SetActiveView(vwTab1_Eligibility)
    ...
    Private Sub InitializeTechnicalForNoProcurement()
        grdBidderList.DataSource = Nothing ' Explicitly set to Nothing for clarity
        grdBidderList.DataBind()
        grdProjectRequirements.DataSource = CreateTable3(9)
        grdProjectRequirements.DataBind()
        grdOtherCriteria.DataSource = Nothing ' Explicitly set to Nothing for clarity
        grdOtherCriteria.DataBind()
        btnSaveTechDocs.Enabled = False
        mvTabs.SetActiveView(vwTab2_Technical)
    End Sub

    Private Sub InitializeSummaryForNoProcurement()
        grdSummary.DataSource = CreateTable4(1)
        grdSummary.DataBind()
        btnSaveSummary.Enabled = False
        mvTabs.SetActiveView(vwTab3_Summary)
    ...
    Protected Sub drpForm2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            ' New check: if txtBidSecAmt_1 is greater than txtReqBidSec_1
            If CType(grdBidderList.Rows(gvr.RowIndex).FindControl("txtBidSecAmt_1"), TextBox).Text > CType(grdBidderList.Rows(gvr.RowIndex).FindControl("txtReqBidSec_1"), TextBox).Text Then
                CType(grdBidderList.Rows(gvr.RowIndex).FindControl("lblStatus"), Label).Text = "Insufficient"
            ElseIf CType(grdBidderList.Rows(gvr.RowIndex).FindControl("drpForm2"), DropDownList).SelectedItem.Value = 6 Then
                CType(grdBidderList.Rows(gvr.RowIndex).FindControl("lblStatus"), Label).Text = "Sufficient"
            ElseIf CType(grdBidderList.Rows(gvr.RowIndex).FindControl("drpForm2"), DropDownList).SelectedItem.Text = "Select" Then
                If CType(CType(grdBidderList.Rows(gvr.RowIndex).FindControl("txtReqBidSec_1"), TextBox).Text, Decimal) < TenPercent Then
                    CType(grdBidderList.Rows(gvr.RowIndex).FindControl("lblStatus"), Label).Text = "Insufficient"
        ...
        Catch ex As Exception
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Something went wrong during the process, please contact the system admin.")
        End Try
    End Sub
