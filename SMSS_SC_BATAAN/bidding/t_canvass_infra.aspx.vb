...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            End If
            grdDirectContract.DataSource = pShopping
            grdDirectContract.DataBind()

            grdDCItems.DataSource = Nothing
            grdDCItems.DataBind()
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        grdDirectContract.DataSource = myview
        grdDirectContract.DataBind()
    ...
    Protected Sub grdDirectContract_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        If Session("Action") = "PRNumber" Then
            dtItems = objDerived.GetDataTable("SELECT * FROM [dbo].[View_DC_ItemList] WHERE prhdr_id = '" & grdDirectContract.SelectedDataKey("prhdr_id") & "'", CommandType.Text)
            grdDCItems.DataSource = dtItems
            grdDCItems.DataBind()

            Dim cb As CheckBox
            For i As Long = 0 To Me.grdDCItems.Rows.Count - 1
                cb = CType(Me.grdDCItems.Rows(i).Cells(0).FindControl("CheckBox1"), CheckBox)
                cb.Checked = True
            Next
        ...
        ElseIf Session("Action") = "Cancel" Then
            Try
                Dim dt As New DataTable
                dt = objDerived.GetDataTable("SELECT * FROM [dbo].[View_Return_Canvass] WHERE prhdr_id = '" & grdDirectContract.SelectedDataKey("prhdr_id") & "'", CommandType.Text)

                If dt.Rows(0)("Hdr_ID") = 0 Then

                    '======= UPDATE AMS.PR_Hdr (mode_of_procurement_id)
                    objDerived.GetRecords("UPDATE AMS.PR_Hdr SET mode_of_procurement_id = 0,isOnBid = 0 WHERE prhdr_id = '" & grdDirectContract.SelectedDataKey("prhdr_id") & "'", CommandType.Text)
    ...
    Protected Sub btnsupplier_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        With cnvss_hdr
            ...
            .PR_Hdr_ID = grdDirectContract.SelectedDataKey("prhdr_id")
            .withWinner = True
            .isDBM = False
        End With

        Dim Hdr_ID As Long
        Hdr_ID = objDerived.GetValue("SELECT Hdr_ID FROM AMS.m_Canvass_Hdr WHERE PR_Hdr_ID = '" & grdDirectContract.SelectedDataKey("prhdr_id") & "' AND isDBM = 0", CommandType.Text)
        ...
        For i As Integer = 0 To grdDCItems.Rows.Count - 1
            cb = CType(Me.grdDCItems.Rows(i).Cells(0).FindControl("CheckBox1"), CheckBox)
            If cb.Checked = True Then
                Dim CanvassPrice As Decimal = CType(CType(grdDCItems.Rows(i).FindControl("txtCost"), TextBox).Text, Decimal)
                Dim CanvassQty As Decimal = CType(CType(grdDCItems.Rows(i).FindControl("lblqty"), Label).Text, Decimal)
    ...
    Protected Sub LoadtxtCostItems()
        ...
        For i As Integer = 0 To grdDCItems.Rows.Count - 1
            cb = CType(Me.grdDCItems.Rows(i).Cells(0).FindControl("CheckBox1"), CheckBox)
            If cb.Checked = True Then
                Dim txtCost As TextBox = CType(grdDCItems.Rows(i).FindControl("txtcost"), TextBox)
                Dim lblqty As Label = CType(grdDCItems.Rows(i).FindControl("lblqty"), Label)

                Dim Tcost As Decimal = FormatNumber(txtcost.Text * lblqty.Text, 2)

                CType(grdDCItems.Rows(i).FindControl("lbltotalx"), Label).Text = FormatNumber(Tcost, 2)
                x = x + (txtcost.Text * lblqty.Text)
            Else
                CType(grdDCItems.Rows(i).FindControl("lbltotalx"), Label).Text = "0.00"
            End If
        Next

        CType(grdDCItems.FooterRow.Cells(4).FindControl("lbltotal"), Label).Text = FormatNumber(x, 2)
    ...
    Protected Sub grdDirectContract_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs)
        pShopping = objDerived.GetDataTable("EXEC [AMS].[sp_CanvassList_Direct]", CommandType.Text)
        If pShopping.Rows.Count < 8 Then
            pShopping.Merge(createdatatable1(7 - pShopping.Rows.Count))
        End If
        grdDirectContract.PageIndex = e.NewPageIndex
        grdDirectContract.DataSource = pShopping
        grdDirectContract.DataBind()
