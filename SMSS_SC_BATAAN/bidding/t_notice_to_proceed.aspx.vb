...
Partial Class t_notice_to_proceed
...
#End Region
    ...
    '        pProject = objDerived.GetDataTable("EXEC [AMS].[sp_NoticeToProceed]", CommandType.Text)
    ...
    Private Sub InitializeDateField()
        ' Set the current date in txtDateProceed
        txtDateProceed.Text = Date.Today.ToString("MM/dd/yyyy")
    End Sub

    Private Sub LoadProjectsData()
        ' Load project data for the Notice to Proceed
        Dim pProject As DataTable = objDerived.GetDataTable("EXEC [AMS].[sp_NoticeToProceed]", CommandType.Text)
        If pProject IsNot Nothing AndAlso pProject.Rows.Count < 5 Then
            ' Ensure there are always at least five rows for formatting purposes
    ...
    Private Sub PopulateSignatoryDropdown()
        ' Populate the ddApprovedBy dropdown with signatories
        Dim dt As DataTable = objDerived.GetDataTable("EXEC [AMS].[sp_NoticeOfAward_Signatory]", CommandType.Text)
        With ddApprovedBy
            .DataSource = dt
            .DataTextField = "SignatoryName"
            .DataValueField = "SignatoryPosition"
            .DataBind()
