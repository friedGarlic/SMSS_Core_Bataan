...
Partial Class bidding_RQ_BAC_Resolution
    ...
    Private Property dtCanvass() As DataTable
        Get
            Return CType(Session("dtCanvass"), DataTable)
        End Get
        Set(ByVal value As DataTable)
            Session("dtCanvass") = value
        End Set
    End Property
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            dtCanvass = objDerived.GetDataTable("EXEC [AMS].[sp_BACResolution_Canvass]", CommandType.Text)
            grdCanvass.DataSource = dtCanvass
            grdCanvass.DataBind()
        ...
        txtCanvassSearch.Attributes.Add("onkeypress", "return fun1(event, '" & btnCanvassSearch.ClientID & "')")
    End Sub
    ...
    Protected Sub grdCanvass_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdCanvass.SelectedIndexChanged

        Session("Page") = "Reports"
        Session("Hdr_ID") = grdCanvass.SelectedDataKey("Hdr_ID")
        Session("Supplier_ID") = grdCanvass.SelectedDataKey("Supplier_ID")

        Me.Page.Response.Redirect("~/bidding/BACResolution_repeat.aspx")

    End Sub

    Protected Sub grdCanvass_PageIndexChanging(sender As Object, e As GridViewPageEventArgs) Handles grdCanvass.PageIndexChanging

        grdCanvass.PageIndex = e.NewPageIndex
        grdCanvass.DataSource = dtCanvass
        grdCanvass.DataBind()

    End Sub

    Protected Sub btnCanvassSearch_Click(sender As Object, e As EventArgs) Handles btnCanvassSearch.Click
        Try
            Dim myview As DataView
            myview = dtCanvass.DefaultView

            If drpCanvassSearch.SelectedItem.Value = 1 Then
                myview.RowFilter = "OBR_No like '%" & txtCanvassSearch.Text & "%'"
            ElseIf drpCanvassSearch.SelectedItem.Value = 2 Then
                myview.RowFilter = "pr_no like '%" & txtCanvassSearch.Text & "%'"
            End If

            grdCanvass.DataSource = myview
            grdCanvass.DataBind()
        Catch ex As Exception
    ...
    Protected Sub btnTab3_Click(sender As Object, e As EventArgs) Handles btnTab3.Click
        btnTab1.CssClass = "TabButton_InActive"
        btnTab2.CssClass = "TabButton_InActive"
        btnTab3.CssClass = "TabButton_Active"

        Me.mvTabs.SetActiveView(Me.vwTab3)
