...
Partial Class bidding_Pre_Procurement
    ...
    Protected Sub LoadTab()
        If btnTab1.CssClass = "TabButton_Active" And btnTab2.CssClass = "TabButton_InActive" Then
            ...
            dtITB = objDerived.GetDataTable("EXEC [AMS].[sp_PreProcurement_ITB_v2]", CommandType.Text)
    ...
    Protected Sub LoadClearITB()
        ...
        txtITB_No.Text = ""
        txtPreBid_ConferenceTime.Text = "8:00 PM"
        txtPreBid_ConferencePlace.Text = ""
        txtBidOpening_Time.Text = "8:00 PM"
    ...
    Protected Sub CheckITB(sender As Object, e As EventArgs)
        ...
        If cb.Checked = True Then
            ...
            'Modified query to check both OBR_No and prhdr_id
            Dim ID As Integer = objDerived.GetValue("SELECT ITB_Hdr_ID FROM AMS.ITB_Dtl WHERE OBR_No = '" & obr_no & "' AND prhdr_id = '" & prhdr_id & "'", CommandType.Text)
            ...
            If ID <> 0 Then
                ...
                tbl1 = objDerived.GetDataTable("SELECT * FROM [AMS].[ITB_Hdr] WHERE ITB_Hdr_ID='" & ID & "'", CommandType.Text)
                ...
                For i As Integer = 0 To tbl1.Rows.Count - 1
                    ...
                    txtProjectName_new.Text = tbl1.Rows(i)("Project_Name")
                Next
    ...
    Private Sub btnSaveITB_Click(sender As Object, e As EventArgs) Handles btnSaveITB.Click
        ...
        If x = 1 Then
            Try
                ...
                Dim ID As Integer = objDerived.GetValue("SELECT ITB_Hdr_ID FROM AMS.ITB_Hdr WHERE ITB_No = '" & txtITB_No.Text & "'", CommandType.Text)
                If ID = 0 Then
                    '====== SAVE ITB HEADER
                    objDerived.Execute("INSERT INTO [AMS].[ITB_Hdr] ([ITB_No],[ITB_Date],[PhilGeps_DateFrom],[PhilGeps_DateTo],[BidForm_AvailabilityDate],[withPreBidConference]    " &
                                          ...
                                          "  '" & txtBidForm_AvailDate.Text & "','" & cbPreBidConference.Checked & "','" & txtPreBid_ConferenceDate.Text & "','" & txtPreBid_ConferenceTime.Text & "', " &
                                          "  '" & replaceapostrophe(txtPreBid_ConferencePlace.Text) & "','" & txtBidOpening_Date.Text & "','" & txtBidOpening_Time.Text & "','" & replaceapostrophe(txtBidOpening_Place.Text) & "','" & txtProjectName_new.Text & "')", CommandType.Text)

                    Dim hdr_id As Long = objDerived.GetValue("SELECT TOP(1) ITB_Hdr_ID FROM AMS.ITB_Hdr ORDER BY ITB_Hdr_ID DESC", CommandType.Text)
                    Session("ITB_Hdr_ID") = hdr_id
                    AddTrace("on  hdr_id: ITB_Hdr_ID: " & Session("ITB_Hdr_ID"))
                    ...
                    For i As Integer = 0 To grdITB.Rows.Count - 1
                        If CType(grdITB.Rows(i).FindControl("cb1"), CheckBox).Visible = True Then
                            If CType(grdITB.Rows(i).FindControl("cb1"), CheckBox).Checked = True Then
                                ...
                                objDerived.Execute("INSERT INTO [AMS].[ITB_Dtl] ([ITB_Hdr_ID],[OBR_No],[prhdr_id],[ProjectName],[ABC],[Sub_ITBNumb]) " &
                ...
                Else
                    '====== UPDATE DETAIL
                    objDerived.Execute("UPDATE [AMS].[ITB_Hdr] SET [ITB_Date]='" & txtITB_Date.Text &
                    ...
                    Session("ITB_Hdr_ID") = ID
                    AddTrace("on  ID2: ITB_Hdr_ID: " & Session("ITB_Hdr_ID"))
                    ...
                    For i As Integer = 0 To grdITB.Rows.Count - 1
                        If CType(grdITB.Rows(i).FindControl("cb1"), CheckBox).Visible = True Then
                            If CType(grdITB.Rows(i).FindControl("cb1"), CheckBox).Checked = True Then
                                ...
                                objDerived.Execute("UPDATE [AMS].[ITB_Dtl] SET [ProjectName]='" &
    ...
    Private Sub btnPreviewITB_Click(sender As Object, e As EventArgs) Handles btnPreviewITB.Click
        ...
        Session("ITB") = "NotLimited"

        AddTrace("ITB_Hdr_ID: " & Session("ITB_Hdr_ID"))
    ...
    Protected Sub loadPreprocurement()
        dtPreProcurement = objDerived.GetDataTable("SELECT DISTINCT " &
                                                   "A.ITB_Date, " &
                                                   "CASE WHEN (SELECT COUNT(X.ITB_Dtl_ID) FROM AMS.ITB_Dtl AS X WHERE X.ITB_Hdr_ID = A.ITB_Hdr_ID) = 1 THEN A.ITB_No ELSE E.Sub_ITBNumb END AS ITB_No, " &
                                                   "B.ProjectName, B.ABC, A.BidOpening_Date, A.BidOpening_Time, " &
                                                   "A.BidOpening_Place, C.obr_evaluation_hdr_id, C.obr_evaluation_dtl_id," &
                                                   "A.ITB_Hdr_ID, B.ITB_Dtl_ID, D.Transaction_type, " &
                                                   "CONVERT(BIT, 1) AS isVisible, A.Project_name " &
                                                   "FROM AMS.ITB_Hdr AS A INNER JOIN AMS.ITB_Dtl B ON A.ITB_Hdr_ID = B.ITB_Hdr_ID " &
                                                   "INNER JOIN AMS.obr_evaluation_dtl C ON B.prhdr_id = C.prhdr_id INNER JOIN AMS.PR_Hdr D ON C.prhdr_id = D.prhdr_id INNER JOIN AMS.ITB_Dtl E ON A.ITB_Hdr_ID = E.ITB_Hdr_ID AND D.prhdr_id = E.prhdr_id " &
                                                   "WHERE C.obr_evaluation_dtl_id NOT IN (SELECT obr_evaluation_dtl_id FROM AMS.pre_procurement X INNER JOIN AMS.pre_procurement_dtl Y ON X.pre_procurement_hdr_id = Y.pre_procurement_hdr_id WHERE withBid = 1) AND D.mode_of_procurement_id = 1 " &
                                                   "ORDER BY A.ITB_Date DESC, ITB_No, B.ProjectName", CommandType.Text)
    ...
    Private Sub btnSavePreProc_Click(sender As Object, e As EventArgs) Handles btnSavePreProc.Click
        Try
            ' First check if record with this ITB_No already exists
            Dim existingHdrId As Long = objDerived.GetValue(
            "SELECT pre_procurement_hdr_id FROM AMS.pre_procurement WHERE project_reference_no = '" &
            grdPreProcurement.SelectedDataKey("ITB_No") & "'",
            CommandType.Text)
            ...
            With hdr
                ...
                .project_name = grdPreProcurement.SelectedDataKey("Project_name")
            ...
            If existingHdrId > 0 Then
                ' Update existing record
                hdr.pre_procurement_hdr_id = existingHdrId
                hdrid = hdr.update()

                ' Delete existing details before inserting new ones
                objDerived.GetRecords("DELETE FROM AMS.pre_procurement_dtl WHERE pre_procurement_hdr_id = " & existingHdrId, CommandType.Text)
            Else
                ' Create new record
                hdrid = hdr.save()
            End If
            ...
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Transaction has been successfully " & If(existingHdrId > 0, "updated", "saved") & ".")
