...
Partial Class Inventory_Disposal_Disposal_Notice
    ...
    Private Sub grdJEV_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdJEV.SelectedIndexChanged
        ...
        dtItems = objDerived.GetDataTable("SELECT DISTINCT A.IsspHdr_ID, A.QuotationHdr_ID, B.IIRUPHdr_ID, B.Property_No, B.BidAmt, D.isDonated, D.Item_ID, C.SerialNo              " &
                                                " FROM AMS.tbl_QuotationHdr AS A INNER JOIN AMS.tbl_QuotationDtl AS B ON A.QuotationHdr_ID = B.QuotationHdr_ID                      " &
                                                " INNER JOIN AMS.Property_Dtl AS C ON B.Property_No = C.PropertyNo INNER JOIN AMS.Property AS D ON C.Property_ID = D.Property_ID    " &
                                                " WHERE A.isWinner = 1 AND A.IsspHdr_ID = '" & grdJEV.SelectedDataKey("IsspHdr_ID") & "' AND A.QuotationHdr_ID = '" & grdJEV.SelectedDataKey("QuotationHdr_ID") & "' AND A.Supplier_ID = '" & grdJEV.SelectedDataKey("Supplier_ID") & "' ORDER BY B.Property_No", CommandType.Text)
