...
#End Region
    ...
    Protected Sub btnsavedoc_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Session("MREHdr_ID_1") = objDerived.GetValue("SELECT MREHdr_ID FROM AMS.MRE_Hdr WHERE MRENumber = '" & txtMRE.Text & "'", CommandType.Text)
    ...
    Protected Sub btnpreviewAreDoc_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ElseIf btnpreviewAreDoc.Text = "PREVIEW PARE" Then
            ...
            ' Retrieve MREHdr_ID from session
            ...
            Else
                AddTrace("Error: MREHdr_ID is missing in Session.")
                ScriptManager.RegisterStartupScript(Me, GetType(String), "NO_MREHDR_ID", "alert('Error: No valid MREHdr_ID found. Please save the document first.');", True)
                Exit Sub
            End If

            ' Validate if MREHdr_ID is set
            If selectedMREHdr_ID Is Nothing OrElse Convert.ToInt64(selectedMREHdr_ID) <= 0 Then
                AddTrace("Error: MREHdr_ID is invalid, aborting operation.")
                ScriptManager.RegisterStartupScript(Me, GetType(String), "INVALID_MREHDR_ID", "alert('Error: Invalid MREHdr_ID. Please try again.');", True)
                Exit Sub
            End If

            ' Open the acknowledgment receipt report with the correct MREHdr_ID
            Dim url As String = "t_rpt_acknowledgement_receipt.aspx?MREHdr_ID=" & selectedMREHdr_ID.ToString()
            Dim fullURL As String = "window.open('" & url + "', '_blank', 'status=0,screenX=0,resizable=1,scrollbars=1,width=850,height=700,left=250,top=100');"
