...
#End Region
    ...
    Protected Sub drpFunction_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles drpFunction.SelectedIndexChanged
        Try
            ...
            ddReceive.DataSource = objDerived.GetDataTable("SELECT * FROM HRMS.view_signatory WHERE deptid ='" & drpSearh_Department.SelectedItem.Value & "' AND division_key = '" & Me.drpFunction.SelectedItem.Value & "' and isDeptHead = 'YES' ORDER BY isDeptHead DESC, full_name", CommandType.Text)
            ddReceive.DataTextField = ("full_name")
            ddReceive.DataValueField = ("empid")
            ddReceive.DataBind()
            ddReceive.Items.Insert(0, "Select")
    ...
    Protected Sub btnsave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnsave.Click
        'Here 1
        Try
            Dim A As Integer = pbody.Compute("sum(Issuance)", "")
            If pbody.Compute("sum(Issuance)", "") = 0 Then
            ...
            Else
                ...
                Dim hdrid As Long = hdr.saveRISHdr()
                objDerived.Execute("UPDATE AMS.RIS_Hdr Set withICS = 0, SAI_No = '" & txtSAI_No.Text & "', SAI_Date = '" & IIf(txtSAI_Date.Text = "", "01/01/1900", txtSAI_Date.Text) & "' WHERE RISHdr_ID = '" & hdrid & "'", CommandType.Text)
                ...
                For i As Integer = 0 To Me.gvbody.Rows.Count - 1
                    If pbody.Rows(i)("Issuance") <> "0" Then
                        ...
                        dtl.AvailableQty = pbody.Rows(i)("Balance")
                        dtl.ApprovedQty = pbody.Rows(i)("Issuance")
                        ...
                        objDerived.GetRecords("update ams.stock set Balance='" & balance - pbody.Rows(i)("Issuance") & "',Issuance='" & issuance + pbody.Rows(i)("Issuance") & "' where stockID =" & pbody.Rows(i)("StockID") & "", CommandType.Text)
                        ...
                        With objStockLedger
                            ...
                            .CreditQty = pbody.Rows(i)("Issuance")
                            ...
                            .BalanceQty = SuppQty - CType(pbody.Rows(i)("Issuance"), Integer)
            ...
            End If
        Catch ex As Exception
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel3, "Please contact the administrator")
        End Try
    End Sub
    ...
    Protected Sub LoadItemList()
        ...
        If ddSupplies.SelectedItem.Text = "Select" Then
            ...
            If drpSearh_Department.SelectedItem.Text = "LGU Warehouse" Then
                If drpRIS_Warehouse.SelectedItem.Text = "Select" Then
                    MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel3, "Select warehouse.")

                Else
                    dtSupplies = objDerived.GetDataTable("EXEC [AMS].[sp_SupplyList_ForRIS_v2] '" & drpSearh_Department.SelectedItem.Text & "',0,'" & dtGenAccount.Rows(ddSupplies.SelectedIndex - 1)("GA_ID") & "','" & dtGenAccount.Rows(ddSupplies.SelectedIndex - 1)("BGA_ID") & "' " &
                                                            " , '" & IIf(drpRIS_Warehouse.SelectedItem.Text = "Select", 0, drpRIS_Warehouse.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpBay.SelectedItem.Text = "Select", 0, drpBay.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpColumn.SelectedItem.Text = "Select", 0, drpColumn.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpRIS_Floor.SelectedItem.Text = "Select", 0, drpRIS_Floor.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpRIS_Room.SelectedItem.Text = "Select", 0, drpRIS_Room.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpRIS_Shelves.SelectedItem.Text = "Select", 0, drpRIS_Shelves.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpRack.SelectedItem.Text = "Select", 0, drpRack.SelectedItem.Value) & "' " &
                                                            " , '" & IIf(drpBin.SelectedItem.Text = "Select", 0, drpBin.SelectedItem.Value) & "' ", CommandType.Text)
                ...
                LoadDetails_Dropdowns()

            Else
                Dim a As Integer = dtGenAccount.Rows(ddSupplies.SelectedIndex - 1)("GA_ID")
                Dim b As Integer = dtGenAccount.Rows(ddSupplies.SelectedIndex - 1)("BGA_ID")
                dtSupplies = objDerived.GetDataTable("EXEC [AMS].[sp_SupplyList_ForRIS_v2] '" & replaceapostrophe(drpSearh_Department.SelectedItem.Text) & "','" & drpSearh_Department.SelectedItem.Value & "','" & dtGenAccount.Rows(ddSupplies.SelectedIndex - 1)("GA_ID") & "','" & dtGenAccount.Rows(ddSupplies.SelectedIndex - 1)("BGA_ID") & "',0,0,0,0,0,0,0,0", CommandType.Text)
                If dtSupplies.Rows.Count < 5 Then
                        dtSupplies.Merge(CreateDataTable_Supplies(5 - dtSupplies.Rows.Count))
                    End If
                    gvSupplyList.DataSource = dtSupplies
                    ...
                    If drpSearh_Department.SelectedItem.Value = 7 Then
                        drpdept.DataSource = objDerived.GetDataTable("SELECT DISTINCT [RC_id], UPPER([RC_Name]) AS RC_Name FROM [dbo].[View_RespCenter_withFunctions] ORDER BY [RC_Name]", CommandType.Text)
                        drpdept.DataTextField = ("RC_Name")
                        drpdept.DataValueField = ("RC_ID")
                        drpdept.DataBind()
                    Else
                        drpdept.DataSource = objDerived.GetDataTable("SELECT DISTINCT [RC_id], UPPER([RC_Name]) AS RC_Name FROM [dbo].[View_RespCenter_withFunctions] WHERE RC_ID = '" & drpSearh_Department.SelectedItem.Value & "' ORDER BY [RC_Name]", CommandType.Text)
                        drpdept.DataTextField = ("RC_Name")
                        drpdept.DataValueField = ("RC_ID")
                        drpdept.DataBind()
                    End If
                    ...
                    drpFunction.DataSource = objDerived.GetDataTable("SELECT DISTINCT [RC_id],[Function_ID],UPPER([Function_Desc]) AS Function_Desc FROM [dbo].[View_RespCenter_withFunctions] WHERE RC_ID = '" & drpSearh_Department.SelectedItem.Value & "' ORDER BY [Function_Desc]", CommandType.Text)
                    drpFunction.DataTextField = ("Function_Desc")
                    drpFunction.DataValueField = ("Function_ID")
                    drpFunction.DataBind()
                    drpFunction.SelectedValue = 86

                    ddmr.DataSource = objDerived.GetDataTable("SELECT * FROM HRMS.view_signatory WHERE deptid ='" & drpSearh_Department.SelectedItem.Value & "' AND division_key = 86 ORDER BY isDeptHead DESC, full_name", CommandType.Text)
                    ddmr.DataTextField = ("full_name")
                    ddmr.DataValueField = ("empid")
                    ddmr.DataBind()

                'ddReceive.DataSource = objDerived.GetDataTable("SELECT * FROM HRMS.view_signatory WHERE deptid ='" & drpSearh_Department.SelectedItem.Value & "' AND division_key = 86 ORDER BY isDeptHead DESC, full_name", CommandType.Text)
                'ddReceive.DataTextField = ("full_name")
                'ddReceive.DataValueField = ("empid")
                'ddReceive.DataBind()
                ddReceive.DataSource = objDerived.GetDataTable("SELECT * FROM HRMS.view_signatory WHERE deptid ='" & drpSearh_Department.SelectedItem.Value & "' AND division_key = '86' and isDeptHead = 'YES' ORDER BY isDeptHead DESC, full_name", CommandType.Text)
                ddReceive.DataTextField = ("full_name")
                ddReceive.DataValueField = ("empid")
                ddReceive.DataBind()
    ...
    Protected Sub cbRISItem_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            If cb.Checked = True Then
                ...
                dtSupplies.Rows(gvSupplyList.Rows(gvr.RowIndex).Cells(6).Text)("isUsed") = True
    ...
    End Sub
    Public Sub New_Saving()
        Try

            '====== CHECK IF RIS NUMBER IS ALREADY EXISTING ======
            Dim RIS As Integer
            RIS = objDerived.GetValue("SELECT [RISHdr_ID] FROM [AMS].[RIS_Hdr] WHERE [RIS_No] = '" & txtRIS.Text & "'", CommandType.Text)
            If RIS <> 0 Then
                MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel3, "RIS already exist.")
            ...
            Else
                ...
                If drpdept.SelectedItem.Text = "Select" Or drpFunction.SelectedItem.Text = "Select" Or ddmr.SelectedItem.Text = "Select" Or ddIssuedby.SelectedItem.Text = "Select" Or ddReceive.SelectedItem.Text = "Select" Then
                    MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel3, "Complete the dropdown details.")
                    Exit Sub
                End If
                '--- CHECK IF THERE ARE QUANTITY ENCODED IN EACH ITEMS

                For a As Integer = 0 To gvbody.Rows.Count - 1
                    If CType(gvbody.Rows(a).FindControl("txtqty"), TextBox).Text = 0 Or CType(gvbody.Rows(a).FindControl("txtqty"), TextBox).Text = "0.00" Then
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel3, "Quantity and unit price are required.")
                        Exit Sub
                    End If
                Next
                ...
                With hdr
                    .RIS_No = txtRIS.Text
                    .RISDate = txtdate.Text
                    .RC_ID = drpdept.SelectedItem.Value
                    .Func_ID = drpFunction.SelectedItem.Value
                    .Requested_By = ddmr.SelectedItem.Text
                    .Received_By = ddReceive.SelectedItem.Text
                    .Approved_By = drpApprovedBy.SelectedItem.Text
                    .Issued_By = ddIssuedby.SelectedItem.Text
                    .withICS = False
                    .Purpose = txtremarks.Text
                End With

                Session("RISHdr_ID") = hdr.saveRISHdr()
                objDerived.Execute("UPDATE AMS.RIS_Hdr Set withICS = 0, SAI_No = '" & txtSAI_No.Text & "', SAI_Date = '" & IIf(txtSAI_Date.Text = "", "01/01/1900", txtSAI_Date.Text) & "' WHERE RISHdr_ID = '" & Session("RISHdr_ID") & "'", CommandType.Text)
                ...
                For i As Integer = 0 To pbody.Rows.Count - 1
                    ...
                    If CType(gvbody.Rows(i).FindControl("txtqty"), TextBox).Text <> 0 Then
                        With dtl
                            .RISHdr_ID = Session("RISHdr_ID")
                            .Item_ID = pbody.Rows(i)("Item_ID")
                            .AvailableQty = pbody.Rows(i)("Balance")
                            .ApprovedQty = CType(gvbody.Rows(i).FindControl("txtqty"), TextBox).Text
                            .Cost = pbody.Rows(i)("Cost")
                            .StockID = pbody.Rows(i)("StockID")
                            .Remarks = CType(gvbody.Rows(i).FindControl("txtRemarks"), TextBox).Text
                            .saveRISDtl()
                        End With

                        objDerived.Execute("UPDATE AMS.stock SET Balance = '" & CType(pbody.Rows(i)("Balance"), Decimal) - CType(CType(gvbody.Rows(i).FindControl("txtqty"), TextBox).Text, Decimal) & "', Issuance = Issuance + '" & CType(CType(gvbody.Rows(i).FindControl("txtqty"), TextBox).Text, Decimal) & "' WHERE StockID = '" & pbody.Rows(i)("StockID") & "'", CommandType.Text)
                        ...
                        With objStockLedger
                            ...
                            .DebitUnit = "-"
                            .DebitCost = "0.00"
                            .CreditQty = CType(gvbody.Rows(i).FindControl("txtqty"), TextBox).Text
                            .CreditCost = (CType(CType(gvbody.Rows(i).FindControl("txtqty"), TextBox).Text, Decimal) * pbody.Rows(i)("Cost"))
                            .CreditUnit = pbody.Rows(i)("UnitDesc")
                            .BalanceUnit = pbody.Rows(i)("UnitDesc")
                        End With

                        objStockLedger.save()

                        Dim ULife As Integer = objDerived.GetValue("SELECT AMS.item_particular.useful_life FROM AMS.item_particular INNER JOIN dbo.m_item ON AMS.item_particular.item_particular_id = dbo.m_item.item_particular_id WHERE dbo.m_item.Item_ID = '" & pbody.Rows(i)("Item_ID") & "'", CommandType.Text)
                        If ULife <> 0 Then
                            Session("withICS") = True
                        End If
                ...
                '====== UPDATE AMS.RIS_Hdr "withICS" ======
                objDerived.Execute("UPDATE AMS.RIS_Hdr set withICS = '" & Session("withICS") & "' WHERE RISHdr_ID = '" & Session("RISHdr_ID") & "'", CommandType.Text)
        ...
        Catch ex As Exception
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel3, "Something went wrong, please contact system admin.")
        End Try
    End Sub
End Class
