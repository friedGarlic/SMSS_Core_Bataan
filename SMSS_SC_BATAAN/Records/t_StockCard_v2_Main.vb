...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            AddTrace("ddClassification: " & ddClassification.SelectedValue)
            'try defaultvalue
    ...
    Public Sub SelectGLAccount()
        ...
        AddTrace("FMparticularsSupplies SQL: exec ams.FMparticularsSupplies '" & 2 & "','" & 0 & "','" & ddClassification.SelectedValue & "','" & drpSubClass.SelectedIndex & "'")

        ddCategory.DataSource = obj.GetDataTable("exec ams.FMparticularsSupplies '" & 2 & "','" & 0 & "','" & ddClassification.SelectedValue & "','" & drpSubClass.SelectedIndex & "'", CommandType.Text)
    ...
    Public Sub SelectSubClassification()
        ...
        ' Coerce class id to integer; default to 0 if empty/non-numeric
        Dim classId As Integer
        If Not Integer.TryParse(ddClassification.SelectedValue, classId) Then
            classId = 0
        End If

        ' Build third parameter: 0 if first item OR empty OR non-numeric; otherwise actual value
        Dim subId As Integer
        If drpSubClass.SelectedIndex <= 0 OrElse Not Integer.TryParse(drpSubClass.SelectedValue, subId) Then
            subId = 0
        End If

        AddTrace("sp_Accounts_Category_v1_02152022 SQL: Exec dbo.sp_Accounts_Category_v1_02152022 " & 2 & "," & classId & "," & subId)

        PListofGL = objDerived.GetDataTable(
        "Exec dbo.sp_Accounts_Category_v1_02152022 " & 2 & "," & classId & "," & subId,
        CommandType.Text
        )
    ...
    Protected Sub ddClassification_SelectedIndexChanged(sender As Object, e As EventArgs)
        SelectClassification()
        ' Get the selected classification ID (assuming DataValueField is classificationid)
        Dim classificationId As Integer = CInt(ddClassification.SelectedValue)

        ' Build SQL to get all GA accounts linked to that classification
        Dim sql As String =
        "SELECT DISTINCT g.GA_ID, g.GA_Title " &
        "FROM LnkdSrvrBOSS.GEOBOS.BOS.m_GenAccnt AS g " &
        "INNER JOIN tblclassmatrix AS c ON c.GA_ID = g.GA_ID " &
