...
#End Region
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            If Session("Records") = "Search" Then
                ...
                gvAccount = objDerived.GetDataTable("Exec dbo.sp_Accounts_Category_v1_02152022 '" & 3 & "', '" & drpClassification.SelectedValue & "',  '" & 0 & "' ", CommandType.Text)
                ...
                ' --- ensure at least a default item exists ---
                If ddGlAccount.Items.Count = 0 Then
                    ddGlAccount.Items.Add(New ListItem("N/A", "0"))
                End If

                ' --- select default if none selected ---
                If ddGlAccount.SelectedItem Is Nothing OrElse String.IsNullOrEmpty(ddGlAccount.SelectedValue) Then
                    ddGlAccount.ClearSelection()
                    ddGlAccount.Items.FindByValue("0").Selected = True
                End If
            ...
            Else
                ...
                gvAccount = objDerived.GetDataTable(" Exec dbo.sp_Accounts_Category_v1_02152022 '" & 3 & "','" & drpClassification.SelectedItem.Value & "','" & 0 & "'", CommandType.Text)
                AddTrace("drpClassification" & drpClassification.SelectedItem.Value)
                ddGlAccount.DataSource = CType(gvAccount, DataTable)
                ddGlAccount.DataTextField = ("GA_Title")
                ddGlAccount.DataValueField = ("GA_ID")
                ddGlAccount.DataBind()
    ...
    Protected Sub ddGlAccount_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddGlAccount.SelectedIndexChanged
        ...
        AddTrace("item_particular_id SQL: " & gvsearchproperty.SelectedDataKey("item_particular_id"))
        AddTrace("Item_ID SQL: " & gvsearchproperty.SelectedDataKey("Item_ID"))
        AddTrace("ddGlAccount SQL: " & ddGlAccount.SelectedItem.Value)
        AddTrace("DeclaredOwner SQL: " & gvsearchproperty.SelectedDataKey("DeclaredOwner"))
        AddTrace("Barangay SQL: " & gvsearchproperty.SelectedDataKey("Barangay"))
    End Sub
    ...
    Protected Sub drpClassification_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        Dim dt As DataTable = db.GetDataTable(sql, CommandType.Text)
    ...
    Public Sub multiviewselected()
        ...
        ElseIf drpClassification.selecteditem.text.contains("books") Or drpClassification.selecteditem.text.contains("Books") Then
        ...
        Else
            ...
            ElseIf drpClassification.selecteditem.text.contains("Machine") Then
                ' MACHINERIES
    ...
    Protected Sub loadMachineryMainGrid()
        ...
        dtAccount = objDerived.GetDataTable("exec [dbo].[SMSS_ProtertyRecords_MACHINERY] '" & drpSubClassification.SelectedValue & "', '" & ddGlAccount.SelectedValue & "' ", CommandType.Text)
    ...
    Protected Sub loadEquipmentList() '[dbo].[SMSS_EquipmentList]
        ...
        Dim sql As String = "exec [dbo].[SMSS_PropertyList_v1_11022022_EQUIPMENT] '" _
                    ...
                    & gvsearchproperty.SelectedDataKey("Barangay") & "'"

        AddTrace("Executing SQL: " & sql)

        dtEquipments = objDerived.GetDataTable(sql, CommandType.Text)

        If dtEquipments.Rows.Count = 0 Then
    ...
    Public Sub LoadOfficeEquipment()
        ...
        txtOfficeEquipmentAcqDate.Text = If(IsDBNull(dt.Rows(0)("Property_Date")) OrElse String.IsNullOrWhiteSpace(dt.Rows(0)("Property_Date").ToString()), "", DateTime.Parse(dt.Rows(0)("Property_Date").ToString()).ToString("MM/dd/yyyy"))
        ...
        txtOfficeEquipmentDepValue.Text = FormatNumber(If(IsDBNull(dt.Rows(0)("DepreciationValue")), 0D, CDbl(dt.Rows(0)("DepreciationValue"))), 2)
        txtOfficeEquipmentDepRate.Text = If(IsDBNull(dt.Rows(0)("DepreciationRate")), "", FormatNumber(CDbl(dt.Rows(0)("DepreciationRate")), 2))
    ...
    Protected Sub loadMachineryList()

        ' Build SQL string first for cleaner AddTrace
        Dim sql As String = "exec [dbo].[SMSS_PropertyList_v1_07182022_MACHINE] '" _
                        & gvsearchproperty.SelectedDataKey("item_particular_id") & "','" _
                        & gvsearchproperty.SelectedDataKey("Item_ID") & "','" _
                        & ddGlAccount.SelectedItem.Value & "','" _
                        & gvsearchproperty.SelectedDataKey("DeclaredOwner") & "','" _
        ...
        AddTrace("loadMachineryList SQL > " & sql)

        ' Execute
        dtMachines = objDerived.GetDataTable(sql, CommandType.Text)
    ...
    Protected Sub loadFurnitureList() '[dbo].[FurnitureList]
        ...
        Catch ex As Exception
            ...
            AddTrace("item_particular_id: " & gvsearchproperty.SelectedDataKey("item_particular_id"))
            AddTrace("Item_ID: " & gvsearchproperty.SelectedDataKey("Item_ID"))
            AddTrace("ddGlAccount: " & ddGlAccount.SelectedItem.Value)
            AddTrace("DeclaredOwner: " & gvsearchproperty.SelectedDataKey("DeclaredOwner"))
            AddTrace("Barangay: " & gvsearchproperty.SelectedDataKey("Barangay"))
        End Try
    ...
    Protected Sub loadFurnitureInformation()
        ...
        'AddTrace("PropertyDetai_ID: " & grdfurnitureandfixtures.SelectedDataKey("PropertyDetai_ID"))
        'AddTrace("PropertyDetai_ID: " & grdfurnitureandfixtures.SelectedDataKey("PropertyDetai_ID"))
        Dim dt As New DataTable
        ...
        Else
            ...
            ''txtDepartment.Text = objDerived.GetValue("SELECT DISTINCT UPPER(RC_Name) AS RC_Name FROM dbo.View_RespCenter_withFunctions where RC_ID = " & dt.Rows(0).Item("RC_ID").ToString, CommandType.Text)
            If Not IsDBNull(dt.Rows(0)("RC_ID")) Then
                Dim rcVal As String = dt.Rows(0)("RC_ID").ToString()
                If drpDepartmentFurnifure.Items.FindByValue(rcVal) IsNot Nothing Then
                    drpDepartmentFurnifure.SelectedValue = rcVal
                End If
            End If
...
#End Region
    ...
    Protected Sub LoadUpdateDepreciatedValue()
        ...
        '   Original logic:
        '   ULife = 10
        '   NoYears = Year(Today) - Year(Date_Accepted)
        '   SalValue = IIf(SalvageValue = 0.0, Cost * 0.05, SalvageValue)
        '   DepVRate = (NoYears / ULife) * 100
        '   TDepValue = Cost - (((Cost - SalValue) / ULife) * NoYears)
        '========================================
        Dim sqlEquip As String =
        "UPDATE EI SET " &
        "    EI.SalvageValue = CASE WHEN V.SalvageValue = 0.0 THEN V.Cost * 0.05 ELSE V.SalvageValue END, " &
        "    EI.DepreciationRate = (CAST((YEAR(GETDATE()) - YEAR(V.Date_Accepted)) AS FLOAT) / 10.0) * 100.0, " &
        "    EI.DepreciationValue = V.Cost - (((V.Cost - CASE WHEN V.SalvageValue = 0.0 THEN V.Cost * 0.05 ELSE V.SalvageValue END) / 10.0) * (YEAR(GETDATE()) - YEAR(V.Date_Accepted))) " &
        "FROM AMS.TbEquipment_Info EI " &
        "INNER JOIN dbo.View_EquipmentInformation V ON EI.Property_Dtl_ID = V.Property_Dtl_ID;"

        objDerived.GetRecords(sqlEquip, CommandType.Text)

        ' Keep Depvalueperyear.Text behavior (based on same formula).
        ' In the old code this ended up being the last row’s value;
        ' here we just compute it from an arbitrary sample row.
        Dim dtSampleEquip As DataTable = objDerived.GetDataTable(
        "SELECT TOP 1 Cost, SalvageValue FROM dbo.View_EquipmentInformation ORDER BY Property_Dtl_ID",
        CommandType.Text)

        If dtSampleEquip IsNot Nothing AndAlso dtSampleEquip.Rows.Count > 0 Then
            Dim cost As Double = CDbl(dtSampleEquip.Rows(0)("Cost"))
            Dim sal As Double = CDbl(IIf(dtSampleEquip.Rows(0)("SalvageValue") = 0D, cost * 0.05, dtSampleEquip.Rows(0)("SalvageValue")))
            Depvalueperyear.Text = ((cost - sal) / 10.0).ToString()
        End If
        ...
        '   ULife = 10
        '   NoYears = Year(Today) - Year(Date_Accepted)
        '   Cost = If(IsDBNull(Cost), 0, Cost)
        '   SalValue = SalvageValue
        '   DepVRate = (Cost - SalValue) / ULife
        '   TDepValue = FormatNumber(Cost - (DepVRate * NoYears), 2)
        '   UPDATE TbMachinery_Information.DepreciationValue = TDepValue
        '========================================
        Dim sqlMach As String =
        "UPDATE MI SET " &
        "    MI.DepreciationValue = ROUND( " &
        "        ISNULL(V.Cost, 0) - ( " &
        "           ((ISNULL(V.Cost, 0) - V.SalvageValue) / 10.0) * (YEAR(GETDATE()) - YEAR(V.Date_Accepted)) " &
        "        ), 2) " &
        "FROM AMS.TbMachinery_Information MI " &
        "INNER JOIN dbo.View_MachineryInformation V ON MI.Property_Dtl_ID = V.Property_Dtl_ID;"

        objDerived.GetRecords(sqlMach, CommandType.Text)


        '========================================
        ' 3) FURNITURE AND FIXTURE
        '   Original logic:
        '   ULife = IIf(IsDBNull(useful_life), 5, useful_life)
        '   If ULife = 0 Then ULife = 1
        '   NoYears = Year(Today) - Year(Date_Accepted)
        '   SalValue = IIf(SalvageValue = 0 Or IsDBNull(SalvageValue), 1000, SalvageValue)
        '   DepVRate = (((Cost - SalValue) / ULife) / Cost) * 100
        '   TDepValue = (Cost - SalValue) / ULife
        '   UPDATE TbFurniture_Info.DepreciationValue, DepreciationRate
        '========================================
        Dim sqlFurn As String =
        "UPDATE FI SET " &
        "    FI.DepreciationValue = " &
        "        (V.Cost - CASE WHEN V.SalvageValue IS NULL OR V.SalvageValue = 0 THEN 1000 ELSE V.SalvageValue END) / " &
        "        CASE WHEN ISNULL(V.useful_life, 5) = 0 THEN 1 ELSE ISNULL(V.useful_life, 5) END, " &
        "    FI.DepreciationRate = " &
        "        ((V.Cost - CASE WHEN V.SalvageValue IS NULL OR V.SalvageValue = 0 THEN 1000 ELSE V.SalvageValue END) / " &
        "        CASE WHEN ISNULL(V.useful_life, 5) = 0 THEN 1 ELSE ISNULL(V.useful_life, 5) END / V.Cost) * 100.0 " &
        "FROM AMS.TbFurniture_Info FI " &
        "INNER JOIN dbo.View_FurnitureInformation V ON FI.Property_Dtl_ID = V.Property_Dtl_ID;"

        objDerived.GetRecords(sqlFurn, CommandType.Text)


        '========================================
        ' 4) BUILDING
        '   Your original code had the UPDATE commented out.
        '   Logic is kept but no DB update is executed here either,
        '   so I'm intentionally not adding a set-based UPDATE.
        '========================================
        ' -- no building UPDATE, same as your original


        '========================================
        ' 5) MOTOR VEHICLE
        '   Original UPDATE was commented out as well.
        '   Logic is preserved conceptually but still no DB write.
        '========================================
        ' -- no motor vehicle UPDATE, same as your original
        ...
        '   Original logic:
        '   ULife = 10
        '   NoYears = Year(Today) - Year(Property_Date)
        '   SalValue = Cost * 0.05
        '   DepVRate = (((Cost - SalValue) / ULife) / Cost) * 100
        '   TDepValue = FormatNumber((Cost - SalValue) / ULife, 2)
        '   UPDATE AMS.TbEquipment_Info.DepreciationValue, DepreciationRate
        '========================================
        Dim sqlInfra As String =
        "UPDATE EI SET " &
        "    EI.DepreciationValue = (V.Cost - (V.Cost * 0.05)) / 10.0, " &
        "    EI.DepreciationRate = (((V.Cost - (V.Cost * 0.05)) / 10.0) / V.Cost) * 100.0 " &
        "FROM AMS.TbEquipment_Info EI " &
        "INNER JOIN ams.View_InfrastructureInformation V ON EI.Property_Dtl_ID = V.Property_Dtl_ID;"

        objDerived.GetRecords(sqlInfra, CommandType.Text)
