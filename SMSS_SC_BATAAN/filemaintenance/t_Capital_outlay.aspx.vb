...
#End Region
    ...
    Private Sub BindTxtClassificationDropdown()
        ' Retrieve same data as drpclass
        Dim dtClassification As DataTable = objDerived.GetDataTable("SELECT ClassificationID, ClassificationName FROM dbo.tbl_Classification", CommandType.Text)

        TxtClassification.DataSource = dtClassification
        TxtClassification.DataTextField = "ClassificationName"
        TxtClassification.DataValueField = "ClassificationID"
        TxtClassification.Items.Clear()
        TxtClassification.DataBind()
        TxtClassification.Items.Insert(0, New ListItem("Select", "0"))
    End Sub

    Protected Sub TxtClassification_SelectedIndexChanged(sender As Object, e As EventArgs)
        ' Implement logic when user selects a classification from this new dropdown
        ' You can reuse the same logic as drpclass_SelectedIndexChanged if needed:
        Dim selectedValue As String = TxtClassification.SelectedItem.Value
        ' Example: Show a message or reuse existing drpclass functionality
        AddTrace("Selected Classification ID: " & selectedValue)

        'ModalPopupExtender8.Show()
        ViewState("KeepClassPopupOpen") = True
    End Sub

    Protected Sub Page_PreRender(sender As Object, e As EventArgs) Handles Me.PreRender
        Dim needShow As Boolean = False

        ' Prefer a precise check on the postback source for reliability
        Dim sm = ScriptManager.GetCurrent(Page)
        Dim sourceId As String = Nothing
        If sm IsNot Nothing AndAlso sm.IsInAsyncPostBack Then
            sourceId = sm.AsyncPostBackSourceElementID
        Else
            sourceId = Request("__EVENTTARGET")
        End If

        If Not String.IsNullOrEmpty(sourceId) Then
            If sourceId.EndsWith("$DropGA") OrElse sourceId.EndsWith("$TxtClassification") _
           OrElse sourceId.EndsWith("$WithSubClass") OrElse sourceId.EndsWith("$SrchClassification") Then
                needShow = True
            End If
        End If

        If Object.Equals(ViewState("KeepClassPopupOpen"), True) Then
            needShow = True
            ViewState("KeepClassPopupOpen") = Nothing
        End If
    ...
    Protected Sub gvstock_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        Else
            If drpclass.SelectedItem.Value = 7 Or drpclass.Text = "Medicine" Then
                ...
                Dim key As DataKey = gvstock.SelectedDataKey

                TextBoxGen.Text = key("GenericName").ToString()
                txtdescription.Text = key("itemdesc").ToString()
                txtItemCode.Text = key("Item_Code").ToString()

                ' Use the right price field – likely the current year price is price2
                txtprice.Text = FormatNumber(CDec(key("price2")), 2)
            ...
            Else
                ...
                ddParticular.SelectedValue = objDerived.GetValue("Select item_particular_id from DBO.m_item where item_ID = '" & gvstock.SelectedDataKey(4) & "'", CommandType.Text)
    ...
    Protected Sub btnadd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnadd.Click
        Try
            If GenAccnt.SelectedIndex = 0 Then
    ...
    Protected Sub btnsave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnsave.Click
        Try
            ...
            '    MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Please Select a Category.")
            '    Exit Sub
            'End If

            If txtdescription.Text = "" Then
                MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Please Enter valid Item Description.")
                Exit Sub
            End If

            If ddUnit.SelectedItem.Text = "Select" Then
            ...
            AddTrace("Executing: EXEC [AMS].[GA_perClass&SubClass] '" &
             GenAccnt.SelectedItem.Value & "','" &
             0 & "','" &
             drpclass.SelectedItem.Value & "','" &
             d & "','" &
             Session("CYPrev") & "','" &
             Session("CYNow") & "'")
    ...
    Protected Sub Button7_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        pstock = objDerived.GetDataTable("Select * from t_ClassWithSubclass", CommandType.Text)
    ...
    Protected Sub btnsaveparticular_Click1(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Else
            a = DrpSubClass.SelectedItem.Value
        End If
        ' Validate empty or missing input
        If String.IsNullOrWhiteSpace(txtLife.Text) Then
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1,
        "Please enter the useful life of the property.")
            Exit Sub
        End If

        ' Validate numeric input
        Dim usefulLife As Integer
        If Not Integer.TryParse(txtLife.Text.Trim(), usefulLife) Then
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1,
        "Useful life must be a numeric (integer) value.")
            Exit Sub
        End If
    ...
    Protected Sub GenAccnt_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        ' Log stored procedure call and parameters
        AddTrace("Calling stored procedure: ams.FMparticularsSupplies")
        AddTrace("Parameters: GenAccnt=" & GenAccnt.SelectedItem.Value &
         ", DrpClass=" & drpclass.SelectedItem.Value &
         ", a=" & a)

        ' Call stored procedure
        pParticular = objDerived.GetDataTable("exec ams.FMparticularsSupplies '" &
                                      GenAccnt.SelectedItem.Value & "','" &
                                      0 & "','" &
                                      drpclass.SelectedItem.Value & "','" &
                                      a & "'",
        ...
        If ddParticular.SelectedItem.Value = "Select" Or ddParticular.SelectedItem.Value = "" Then
        ...
        ' Build SQL string for FMparticularsSupplies
        Dim sqlParticular As String = "EXEC [AMS].[FMparticularsSupplies] " &
                              GenAccnt.SelectedItem.Value & "," &
                              0 & "," &
                              drpclass.SelectedItem.Value & "," &
                              a
        AddTrace(sqlParticular)

        pParticular = objDerived.GetDataTable(sqlParticular, CommandType.Text)
        AddTrace("Rows returned: " & If(pParticular IsNot Nothing, pParticular.Rows.Count.ToString(), "0"))

        ' ========= RETRY LOGIC FOR GA_perClass&SubClass =========

        ' 1) Try with the selected SubClassificationID (d)
        Dim sqlStockBase As String = "EXEC [AMS].[GA_perClass&SubClass] " &
                             GenAccnt.SelectedItem.Value & "," &
                             0 & "," &
                             drpclass.SelectedItem.Value & ",{0}," &
                             Session("CYPrev") & "," &
                             Session("CYNow")

        Dim sqlStock As String = String.Format(sqlStockBase, d)
        AddTrace(sqlStock)
        pstock = objDerived.GetDataTable(sqlStock, CommandType.Text)
        AddTrace("Rows returned: " & If(pstock IsNot Nothing, pstock.Rows.Count.ToString(), "0"))

        Dim DDSubclass As String

        If DrpSubClass.SelectedItem Is Nothing OrElse DrpSubClass.SelectedItem.Text.Trim() = "" OrElse DrpSubClass.SelectedItem.Text.Trim().ToUpper() = "SELECT" Then
            DDSubclass = "0"
        Else
            DDSubclass = DrpSubClass.SelectedItem.Text.Trim()
        End If

        ' 2) If no rows, find all SubClassificationIDs with the same name under the same Classification
        If (pstock Is Nothing OrElse pstock.Rows.Count = 0) Then

            Dim dtAlt As DataTable = objDerived.GetDataTable(
        "SELECT SubClassificationID " &
        "FROM dbo.tbl_SubClassification " &
        "WHERE ClassificationID = '" & drpclass.SelectedValue & "' " &
        "AND SubclassificationName = '" & DDSubclass & "'",
        CommandType.Text)

            ' 3) Retry each alternative SubClassificationID until something returns
            If dtAlt IsNot Nothing AndAlso dtAlt.Rows.Count > 0 Then
                For Each r As DataRow In dtAlt.Rows
                    Dim altId As Integer = CInt(r("SubClassificationID"))
                    If altId = d Then Continue For ' already tried

                    Dim sqlTry As String = String.Format(sqlStockBase, altId)
                    AddTrace("Retry: " & sqlTry)

                    pstock = objDerived.GetDataTable(sqlTry, CommandType.Text)
                    AddTrace("Rows returned: " & If(pstock IsNot Nothing, pstock.Rows.Count.ToString(), "0"))

                    If pstock IsNot Nothing AndAlso pstock.Rows.Count > 0 Then
                        Exit For
                    End If
                Next
            End If
    ...
    Protected Sub LinkButton4_Click(sender As Object, e As EventArgs)
        ...
        GvClass.DataSource = objDerived.GetDataTable("Select * from t_ClassWithSubclass order by ClassificationName asc", CommandType.Text)
        GvClass.DataBind()
    ...
    Protected Sub btnSaveClass_Click(sender As Object, e As EventArgs)
        If btnSaveClass.Text = "SAVE" Then
            ...
            AddTrace("TxtClassification.SelectedIndex : " & TxtClassification.SelectedIndex)
            Dim DtGrid = objDerived.GetValue("Select classificationID from dbo.tbl_classification where classificationID ='" & TxtClassification.SelectedIndex & "'", CommandType.Text)
            Dim a As Integer
            If DdSubClassification.Text = "" Or DdSubClassification.Text = "Select" Then
                a = 0
            Else
            ...
            If Me.WithSubClass.Checked = True Then
                ...
                If DtGrid Is Nothing Then
                    ...
                    Dim Cls = objDerived.GetValue("select count(*) from tbl_Classification where classificationID ='" & TxtClassification.SelectedIndex & "'", CommandType.Text)
                    If Cls <> 0 Then
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Classification is already Exist.")
                    Else
                        Dim ClassName = objDerived.GetValue("SELECT TOP 1 ClassificationName FROM dbo.tbl_classification WHERE ClassificationID = " & TxtClassification.SelectedIndex, CommandType.Text)
                        Me.objDerived.Execute("INSERT INTO dbo.tbl_Classification (ClassificationName, AllotmentClass_id) VALUES ('" & ClassName & "', '3')", CommandType.Text)
                        ...
                        Me.objDerived.Execute("Insert into dbo.tblclassmatrix(ClassificationID,SubClassificationID)Values('" & TxtClassification.SelectedIndex & "','" & a & "')", CommandType.Text)
                        GvClass.DataSource = objDerived.GetDataTable("Select * from t_ClassWithSubclass where allotmentClass_ID = 2 order by ClassificationName asc", CommandType.Text)
                        GvClass.DataBind()
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Transaction has been successfully saved.")
                ...
                Else
                    ...
                    Else
                        'Dim Matrx = objDerived.GetValue("Select TOP 1 classificationID from dbo.tbl_classification order by ClassificationID desc ", CommandType.Text)
                        Me.objDerived.Execute("Insert into dbo.tblclassmatrix(ClassificationID,SubClassificationID)Values('" & TxtClassification.SelectedIndex & "','" & a & "')", CommandType.Text)
                        GvClass.DataSource = objDerived.GetDataTable("Select * from t_ClassWithSubclass where allotmentClass_ID = 3 order by ClassificationName asc", CommandType.Text)
                        GvClass.DataBind()
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Transaction has been successfully saved.")
                        TxtClassification.Enabled = False
                        DdSubClassification.Enabled = False
            ...
            Else
                If DtGrid Is Nothing Then
                    ...
                    Else
                        ...
                        Me.objDerived.Execute("Insert into dbo.tblclassmatrix(ClassificationID,GA_ID ,SubClassificationID)Values('" & TxtClassification.SelectedIndex & "','" & DropGA.SelectedItem.Value & "','" & a & "')", CommandType.Text)
                        GvClass.DataSource = objDerived.GetDataTable("Select * from t_ClassWithSubclass where allotmentClass_ID = 3 order by ClassificationName asc", CommandType.Text)
                        GvClass.DataBind()
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Transaction has been successfully saved.")
                        TxtClassification.Enabled = False
        ...
        Else
            objDerived.GetRecords("UPDATE tbl_classification " &
                      "SET ClassificationName = '" & TxtClassification.SelectedItem.Text & "' " &
                      "WHERE ClassificationID = '" & GvClass.SelectedDataKey(0).ToString() & "'", CommandType.Text)

            Dim F As Integer = objDerived.GetValue("Select ClassificationID from tbl_classification where ClassificationID ='" & TxtClassification.SelectedIndex & "'", CommandType.Text)

            GvClassF = objDerived.GetDataTable("Select ClassificationName,ClassificationID from t_ClassWithSubclass where classificationID='" & F & "'", CommandType.Text)
    ...
    Protected Sub ddGASubClass_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddGASubClass.SelectedIndexChanged
        ' Do any logic you need here ...
        ModalPopupExtender7.Show() ' <-- Sub-class modal's extender
    End Sub


    Protected Sub DropGA_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropGA.SelectedIndexChanged
        'ModalPopupExtender8.Show()
        ViewState("KeepClassPopupOpen") = True
    End Sub
    ...
    Protected Sub BtnSave_SUBCLASS_Click(sender As Object, e As EventArgs)
        If BtnSave_SUBCLASS.Text = "SAVE" Then
            ...
            Else
                ...
                AddTrace("GA_ID: " & ddGASubClass.SelectedItem.Value)
                ...
                Else
                    If NewSubClassificationTxt.Text = "" Then
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Sub Classification is Required.")
                    Else
                        '--- 1) Insert new SubClassification ---
                        Dim subName As String = NewSubClassificationTxt.Text.Replace("'", "''")

                        Me.objDerived.Execute(
                            "INSERT INTO dbo.tbl_SubClassification (SubClassificationName, ClassificationID, GA_ID) " &
                            "VALUES ('" & subName & "', '" & ddClassNewSub.SelectedItem.Value & "', '" & ddGASubClass.SelectedItem.Value & "')",
                            CommandType.Text)

                        '--- 2) Get the SubClassificationID of the row just inserted ---
                        Dim newSubClassificationID As Integer = CInt(
                            objDerived.GetValue(
                                "SELECT TOP 1 SubClassificationID " &
                                "FROM dbo.tbl_SubClassification " &
                                "WHERE SubClassificationName = '" & subName & "' " &
                                "  AND ClassificationID = '" & ddClassNewSub.SelectedItem.Value & "' " &
                                "  AND GA_ID = '" & ddGASubClass.SelectedItem.Value & "' " &
                                "ORDER BY SubClassificationID DESC",
                                CommandType.Text))

                        '--- 3) Use that ID in tblclassmatrix insert ---
                        Me.objDerived.Execute(
                            "INSERT INTO dbo.tblclassmatrix (ClassificationID, GA_ID, SubClassificationID) " &
                            "VALUES ('" & ddClassNewSub.SelectedItem.Value & "', '" & ddGASubClass.SelectedItem.Value & "', '" & newSubClassificationID & "')",
                            CommandType.Text)
        ...
        Else
            ...
            'If Update
            If NewSubClassificationTxt.Text Is Nothing OrElse NewSubClassificationTxt.Text.Trim() = "" Then
                MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Sub Classification is required.")
            Else
                ' Keys/values
                Dim subId As Integer = CInt(GvSubClass.SelectedDataKey(0))          ' SubClassificationID
                Dim classId As Integer = CInt(ddClassNewSub.SelectedItem.Value)     ' ClassificationID
                Dim gaId As Integer = CInt(ddGASubClass.SelectedItem.Value)         ' GA_ID
                Dim subName As String = NewSubClassificationTxt.Text.Replace("'", "''")

                ' Single T-SQL batch with explicit transaction and RAISERROR (for older SQL Server)
                Dim sql As String =
                    "BEGIN TRY" & vbCrLf &
                    "    BEGIN TRAN;" & vbCrLf &
                    "" & vbCrLf &
                    "    -- 1) Update the exact SubClassification row" & vbCrLf &
                    "    UPDATE dbo.tbl_SubClassification" & vbCrLf &
                    "    SET SubClassificationName = '" & subName & "'," & vbCrLf &
                    "        GA_ID = " & gaId & vbCrLf &
                    "    WHERE SubClassificationID = " & subId & ";" & vbCrLf &
                    "" & vbCrLf &
                    "    -- 2) Keep the matrix GA aligned to the SubClassification GA for this Class/Sub" & vbCrLf &
                    "    UPDATE dbo.tblclassmatrix" & vbCrLf &
                    "    SET GA_ID = " & gaId & vbCrLf &
                    "    WHERE ClassificationID = " & classId & vbCrLf &
                    "      AND SubClassificationID = " & subId & ";" & vbCrLf &
                    "" & vbCrLf &
                    "    COMMIT;" & vbCrLf &
                    "END TRY" & vbCrLf &
                    "BEGIN CATCH" & vbCrLf &
                    "    IF @@TRANCOUNT > 0 ROLLBACK;" & vbCrLf &
                    "    DECLARE @ErrMsg nvarchar(4000), @ErrSev int, @ErrState int;" & vbCrLf &
                    "    SELECT @ErrMsg = ERROR_MESSAGE(), @ErrSev = ERROR_SEVERITY(), @ErrState = ERROR_STATE();" & vbCrLf &
                    "    RAISERROR(@ErrMsg, @ErrSev, @ErrState);" & vbCrLf &
                    "END CATCH;"

                ' Execute
                objDerived.Execute(sql, CommandType.Text)

                MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Transaction has been successfully saved.")

                ' Refresh grid (keep your existing binding)
                'GvClassF = objDerived.GetDataTable("Exec AMS.FMSubClassification '" & ddClassNewSub.SelectedItem.Value & "'", CommandType.Text)
                GvClassF = objDerived.GetDataTable("Select * from t_ClassWithSubclass", CommandType.Text)
                GvClass.DataSource = GvClassF
                GvClass.DataBind()
            End If
        ...
        'List of Subclassification Rebind
        DrpGenAcc = objDerived.GetDataTable("Select GA_ID, GA_Code + ' - '+ GA_Title as ga_title from ams.vw_supplies  where AllotmentClass_Id = 3 order by GA_Title ", CommandType.Text)
        ddGASubClass.DataSource = DrpGenAcc
        ddGASubClass.DataTextField = "GA_title"
        ddGASubClass.DataValueField = "GA_ID"
        ...
        ' --- Safe grab of values using your style ---
        Dim classVal As String = ""
        Dim subClassVal As String = ""

        If drpclass IsNot Nothing AndAlso drpclass.SelectedItem IsNot Nothing Then
            classVal = drpclass.SelectedValue
        End If

        If DrpSubClass IsNot Nothing AndAlso DrpSubClass.SelectedItem IsNot Nothing Then
            subClassVal = DrpSubClass.SelectedValue
        End If

        If String.IsNullOrEmpty(classVal) OrElse String.IsNullOrEmpty(subClassVal) Then
            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Please select a Class and Subclass first.")
            Return
        End If

        ' --- Your trace, same syntax ---
        AddTrace("Executed: [AMS].[sp_FM_GvClass] null,'" & classVal & "','" & subClassVal & "','" & 0 & "'")

        ' --- Your data call, same syntax (kept exactly like your reference) ---
        DrpGenAcc = objDerived.GetDataTable(" [AMS].[sp_FM_GvClass] null,'" & classVal & "','" & subClassVal & "','" & 0 & "'", CommandType.Text)
    ...
    Protected Sub GvClass_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles GvClass.PageIndexChanging
        ...
        ClassificationGrd = objDerived.GetDataTable("Select * from t_ClassWithSubclass where AllotmentClass_id = 3 order by ClassificationName asc", CommandType.Text)
    ...
    Protected Sub GvClass_SelectedIndexChanged(sender As Object, e As EventArgs)
        ' Get the selected ClassificationName
        Dim selectedClassificationName As String = GvClass.SelectedDataKey("ClassificationName").ToString()

        ' Set the dropdown to the item that matches the selected ClassificationName
        Dim item As ListItem = TxtClassification.Items.FindByText(selectedClassificationName)
        If item IsNot Nothing Then
            TxtClassification.ClearSelection()
            item.Selected = True
        Else
            ' If not found (in case dropdown doesn't have the value)
            TxtClassification.SelectedIndex = 0
        End If
