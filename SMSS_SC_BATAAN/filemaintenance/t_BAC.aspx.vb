...
Partial Class filemaintenance_t_BAC
    ...
    Protected Sub chkViewHidden_CheckedChanged(sender As Object, e As EventArgs)
        Try
            If chkViewHidden.Checked Then
                Dim sql As String = "SELECT AMS.BAC_Members.Name, " &
                                "AMS.BAC_Members.Position, " &
                                "AMS.BAC_position.Position_desc, " &
                                "CASE WHEN AMS.BAC_Members.isPublicInfra = 0 THEN 'Goods' ELSE 'Infrastructure' END AS isPublicInfra, " &
                                "AMS.BAC_position.code, " &
                                "AMS.BAC_Members.empsig_id, " &
                                "AMS.BAC_position.BAC_PostionID, " &
                                "AMS.BAC_Members.id, " &
                                "AMS.BAC_Members.isActive, " &
                                "ISNULL(AMS.BAC_Members.isDefault, 0) AS isDefault " &
                                "FROM AMS.BAC_Members " &
                                "INNER JOIN AMS.BAC_position ON AMS.BAC_Members.BAC_PostionID = AMS.BAC_position.BAC_PostionID " &
                                "WHERE AMS.BAC_Members.isActive = 0"

                Dim dtBAC As DataTable = objDerived.GetDataTable(sql, CommandType.Text)
                grdBAC.DataSource = dtBAC
                grdBAC.DataBind()
            Else
                LoadMembers()
        ...
        Catch ex As Exception
            AddTrace("Error in chkViewHidden_CheckedChanged: " & ex.Message)
        End Try
    End Sub
    ...
    Protected Sub btnSaveNew_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        AddTrace("btnNewPosition: " & btnNewPosition.Text)
        If btnNewPosition.Text = "New" Then

            AddTrace("ddNewBAC: " & ddNewBAC.SelectedItem.Value)
            '=== SAVE NEW BAC
            Dim dt As New DataTable
