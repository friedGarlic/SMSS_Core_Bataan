...
#End Region
    ...
    Protected Sub LoadSignatories()
        ...
        AddTrace("ddDepartment_Search: " & ddDepartment_Search.SelectedItem.Text)
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            ElseIf btnSave.Text = "UPDATE" Then
                ...
                If btnNewPosition.Text = "New" Then
                    '=== UPDATE USING EXISTING POSITION (FROM DROPDOWN) ===
                    If txtName.Text = "" Or ddFunction.SelectedItem.Text = "Select" Or ddPosition.SelectedItem.Text = "Select" Or ddDeptHead.SelectedItem.Text = "Select" Or ddIsActive.SelectedItem.Text = "Select" Then
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "All fields are required.")
                        Exit Sub
                    Else
                        '=== CHECK IF THERE IS ANOTHER DEPT HEAD IN THIS OFFICE ===
                        Dim VerifyHead As Integer = 0
                        If ddDeptHead.SelectedItem.Value = 2 Then
                            VerifyHead = objDerived.GetValue("SELECT TOP(1) EmpID FROM dbo.tb_Signatories " &
                                                             "WHERE isDeptHead = 'Yes' AND isActive = 1 " &
                                                             "AND deptid = '" & ddDepartment.SelectedItem.Value & "' " &
                                                             "AND division_Key = '" & ddFunction.SelectedItem.Value & "' " &
                                                             "AND EmpID <> '" & grdSignatories.SelectedDataKey("EmpID") & "'", CommandType.Text)
                        End If

                        If VerifyHead = 0 Then
                            objDerived.GetRecords("UPDATE [dbo].[tb_Signatories] SET " &
                                                  "[Full_Name] = '" & txtName.Text.ToUpper() & "', " &
                                                  "[Position_ID] = '" & ddPosition.SelectedItem.Value & "', " &
                                                  "[isDeptHead] = '" & ddDeptHead.SelectedItem.Text & "', " &
                                                  "[deptid] = '" & ddDepartment.SelectedItem.Value & "', " &
                                                  "[division_Key] = '" & ddFunction.SelectedItem.Value & "', " &
                                                  "[isActive] = '" & ddIsActive.SelectedItem.Value & "', " &
                                                  "[isInspector] = '" & ddisInspector.SelectedItem.Value & "' " &
                                                  "WHERE [EmpID] = '" & grdSignatories.SelectedDataKey("EmpID") & "'", CommandType.Text)
                ...
                ElseIf btnNewPosition.Text = "Search" Then
                    '=== UPDATE WITH NEW POSITION DESCRIPTION ===
                    If txtName.Text = "" Or ddFunction.SelectedItem.Text = "Select" Or txtPositionDesc.Text = "" Or ddDeptHead.SelectedItem.Text = "Select" Or ddIsActive.SelectedItem.Text = "Select" Then
                        MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "All fields are required.")
                        Exit Sub
                    Else
                        Try
                            Dim Position_ID As Integer
                            Position_ID = objDerived.GetValue("SELECT DISTINCT Position_ID FROM dbo.m_position " &
                                                              "WHERE position_desc LIKE '" & replaceapostrophe(txtPositionDesc.Text) & "'", CommandType.Text)

                            If Position_ID = 0 Then
                                lblNoti.Visible = False

                                '=== SAVE POSITION DETAILS ===
                                objDerived.GetRecords("INSERT INTO dbo.m_position (position_desc,dept_id) " &
                                                      "VALUES('" & replaceapostrophe(txtPositionDesc.Text) & "', '" & ddDepartment.SelectedItem.Value & "')", CommandType.Text)
                                ...
                                If VerifyHead = 0 Then
                                    '=== UPDATE SIGNATORY TO USE NEW POSITION ===
                                    objDerived.GetRecords("UPDATE [dbo].[tb_Signatories] SET " &
                                                          "[Full_Name] = '" & txtName.Text.ToUpper() & "', " &
                            ...
                            End If
                        Catch ex As Exception
                            MsgeBox.CreateMessageAlertInUpdatePanel(Me.UpdatePanel1, "Error occured during updating, contact administrator.")
                        End Try
                    End If
                End If
    ...
    Protected Sub grdSignatories_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdSignatories.SelectedIndexChanged
        ...
        btnNewPosition.Enabled = True
        btnNewPosition.Text = "New"

        ddPosition.Visible = True
        txtPositionDesc.Visible = False

        ' TO AUTO SELECT POSITION
        Dim posDesc As String = grdSignatories.SelectedDataKey("Position_Desc").ToString()
        Dim li As ListItem = ddPosition.Items.FindByText(posDesc)
